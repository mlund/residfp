...
use residfp::sampler::Sampler;
use residfp::{ChipModel, Synth};
...
pub fn bench_compute_convolution_fir(c: &mut Criterion) {
    ...
    c.bench_function("convolution_fir_wide_256", |b| {
        let sampler = Sampler::new(Synth::new(ChipModel::Mos6581));
        let samples = [2i16; 1024];
        let fir = [5i16; 1024];
        b.iter(|| sampler.compute_convolution_fir_wide_256(&samples[..], &fir[..]))
    });
